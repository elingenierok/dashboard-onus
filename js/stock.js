...
async function ejecutarBotHistorico() {
  ...
  // 🛡️ VERIFICACIÓN ANTI-SOBREESCRITURA
  // Se evalúa en Supabase ANTES de abrir Puppeteer
  ...
  if (registroExistente && registroExistente.length > 0) {
    console.log(`⚠️ [HISTÓRICO] Ya existe la foto matutina de hoy (${fechaHoy}). Se cancela para preservar la captura de las 07:30 AM.`);
    return; // Cancela la ejecución sin abrir navegador ni modificar la base
  }
